...
int containsBSTree(struct BSTree *tree, TYPE val)
{
    ...
    while (curNode != 0) {
        ...
        else if (compare(curNode->val, val) == 1) {
            //its more
...
struct Node *_removeNode(struct Node *cur, TYPE val)
{
    ...
    //I was not able to get the compare function to work here
    if (val->number == cur->val->number) {
        if (cur->right == 0) {
            return cur->left;
        }
        else {
    ...
    else if (val->number < cur->val->number) {
...
void testRemoveLeftMost() {
    ...
    printf("There is no leftmost of right of root. Right of root is a leaf Node\n");
