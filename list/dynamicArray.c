...
void _adjustHeap(DynArr *heap, int max, int pos)
{
    ...
    //else no children, we are at the bottom -> done
}
...
void _buildHeap(DynArr *heap)
{
    ...
    for (int i = heap->size; i >= 1; i--) {
        //find the last non-leaf node,i, going form left to right
        int lastNonLeafNode = heap->size / 2 - 1;
        //adjust heap from last non-leaf node to max
        _adjustHeap(heap, heap->size, lastNonLeafNode);
...
{
    /* FIXME */
    //1.BuildHeap - turn arbitarary array into a heap
    _buildHeap(heap);
    ...
    while (last != 0) {
        ...
        swapDynArr(heap, last, 0);
        //adjust heap (from 0 to the last...not inclusive)
        _adjustHeap(heap, last, 0);
        last--;
    }
